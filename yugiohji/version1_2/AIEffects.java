...
public class AIEffects {
    ...
    public static void cpuEffectBigBackBouncerOnMonster (SummonedMonster EffectMonster, SummonedMonster TargetMonster, AIsacrifice Sacrifice1, AIsacrifice Sacrifice2) {
        if (AIsacrifice.sacrificeIsNotTargetMonster(Sacrifice1, Sacrifice2, TargetMonster) && worthInSemipointsOfAllCurrentSacrifices()==2) { // proceed only, if target is not tributed away
            ...
            YuGiOhJi.informationDialog("Computer wants to use its " + getNumberAsString(EffectMonster.sumMonsterNumber) + " monster, " + EffectMonster.Monster.monsterName + ", to return " + pronoun + getNumberAsString(TargetMonster.sumMonsterNumber) + " monster" + monsterName + " back to the hand.", "Effect attempt");
            boolean isCanceling = askPlayerToNegateActivatedEffect(EffectMonster);
            if (!isCanceling) {
                YuGiOhJi.informationDialog("Computer uses its " + getNumberAsString(EffectMonster.sumMonsterNumber) + " monster, " + EffectMonster.Monster.monsterName + ", to return " + pronoun + getNumberAsString(TargetMonster.sumMonsterNumber) + " monster" + monsterName + " back to the hand.", "Returning card");
                Game.deactivateCurrentCPUEffects();
    ...
    public static void cpuEffectBigBackBouncerOnEquipCard (SummonedMonster EffectMonster, EStack Stack, int cardNumberInStack, AIsacrifice Sacrifice1, AIsacrifice Sacrifice2) {
        if (AIsacrifice.sacrificeIsNotTargetedEquipCard(Sacrifice1, Sacrifice2, Stack, cardNumberInStack) && AIsacrifice.sacrificeIsNotEquippedWithTarget(Sacrifice1, Sacrifice2, Stack) && worthInSemipointsOfAllCurrentSacrifices()==2) { // proceed only, if target is not tributed away
            ...
            YuGiOhJi.informationDialog("Computer wants to use its " + getNumberAsString(EffectMonster.sumMonsterNumber) + " monster, " + EffectMonster.Monster.monsterName + ", to return " + pronounEquipCard + possiblyNegated + "equip card (" + Stack.getNthCardOfStack(cardNumberInStack).lowMonster.monsterName + ", in equip stack of " + pronounMonster + getNumberAsString(Stack.stackNumber) + " monster) back to the hand.", "Effect attempt");
            boolean isCanceling = askPlayerToNegateActivatedEffect(EffectMonster);
            if (!isCanceling) {
                YuGiOhJi.informationDialog("Computer uses its " + getNumberAsString(EffectMonster.sumMonsterNumber) + " monster, " + EffectMonster.Monster.monsterName + ", to return " + pronounEquipCard + possiblyNegated + "equip card (" + Stack.getNthCardOfStack(cardNumberInStack).lowMonster.monsterName + ", in equip stack of " + pronounMonster + getNumberAsString(Stack.stackNumber) + " monster) back to the hand.", "Returning card");
                Game.deactivateCurrentCPUEffects();
    ...
    public static void cpuEffectBigBackBouncerOnGYCard (SummonedMonster EffectMonster, int cardIdInGY, AIsacrifice Sacrifice1, AIsacrifice Sacrifice2) {
        if (AIsacrifice.sacrificeIsNotTargetedGYCard(Sacrifice1, Sacrifice2, cardIdInGY) && worthInSemipointsOfAllCurrentSacrifices()==2) { // proceed only, if target is not tributed away
            ...
            YuGiOhJi.informationDialog("Computer wants to use its " + getNumberAsString(EffectMonster.sumMonsterNumber) + " monster, " + EffectMonster.Monster.monsterName + ", to return its GY card (" + GYDeckCPU.getNthCardOfDeck(cardNumberInGY).cardName + ") back to the hand.", "Effect attempt");
            boolean isCanceling = askPlayerToNegateActivatedEffect(EffectMonster);
            if (!isCanceling) {
                YuGiOhJi.informationDialog("Computer uses its " + getNumberAsString(EffectMonster.sumMonsterNumber) + " monster, " + EffectMonster.Monster.monsterName + ", to return its GY card (" + GYDeckCPU.getNthCardOfDeck(cardNumberInGY).cardName + ") back to the hand.", "Returning card");
                Game.deactivateCurrentCPUEffects();
    ...
    public static void cpuEffectMonsterStealerOnMonster (SummonedMonster EffectMonster, SummonedMonster TargetMonster, AIsacrifice Sacrifice1, AIsacrifice Sacrifice2, AIsacrifice Sacrifice3, AIsacrifice Sacrifice4) {
        if (worthInSemipointsOfAllCurrentSacrifices()==4) {
            ...
            Game.isActCPUEff = true;
            EffectMonster.canStillUseOncePerTurnEffect=false;
            EffectMonster.canStillAttackThisTurn=false; // Important!: This line has to be here at the beginning of the effect - not the end in order to prevent the rare bug, that a newly stolen monster couldn't attack, if Monster Stealer sacrificed itself and the stolen monster took its place/slot.
            cpuPaysCost(Sacrifice1, Sacrifice2, Sacrifice3, Sacrifice4);
    ...
    public static void cpuHandTrapEffectNegateOnMonster (SummonedMonster TargetMonster) {
        ...
        if (cpuCanUseHandTrapEffectNegate()) {
            ...
            YuGiOhJi.informationDialog("Computer discards a " + Card.NeutraliserSkillStealer.cardName + " in order to negate the effects of " + pronoun + getNumberAsString(TargetMonster.sumMonsterNumber) + " monster, " + TargetMonster.Monster.monsterName + ".", "Effect negation");
    ...
    public static void cpuSpecialSummonDemonFromStack (EStack Stack, boolean isInAttMode) {
        ...
        if (Stack.numberOfCards>0) {
            ...
            if (cardNumberInStack!=0 && zoneNumber>0) { // proceed only, if CPU has usable Demon in equip stack
                ...
                YuGiOhJi.informationDialog("Computer wants to special summon its equip monster " + Mon.Demon.monsterName + " from " + pronounStack + getNumberAsString(Stack.stackNumber) + " equip stack.", "Effect attempt");
                boolean isCanceling = askPlayerToNegateActivatedEffect(Stack, cardNumberInStack, false);
                if (!isCanceling) {
                    YuGiOhJi.informationDialog("Computer special summons its equip monster " + Mon.Demon.monsterName + " from " + pronounStack + getNumberAsString(Stack.stackNumber) + " equip stack.", "Summoning");
