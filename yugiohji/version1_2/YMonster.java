...
public class YMonster {
    ...
    public static void effectMonsterStealerActivate (SummonedMonster SumMonster) {
        ...
        if (intDialogResult==0) {
            ...
            Game.ActEffMonSource=SumMonster;
            SumMonster.canStillUseOncePerTurnEffect=false;
            // forbid user of the effect to attack this turn
            SumMonster.canStillAttackThisTurn=false; // Important!: This line has to be here at the beginning of the effect - not the end in order to prevent the rare bug, that a newly stolen monster couldn't attack, if Monster Stealer sacrificed itself and the stolen monster took its place/slot.
    ...
    public static void cheatChangeEffectDemon (SummonedMonster SumMonster) {
        int intDialogResult = YuGiOhJi.multipleChoiceDialog("Tribute all other summoned " + Mon.Demon.monsterName + "s you control to cheat mode change this card?", "Are you sure?", new String[]{"yes", "no"}, "no");
        if (intDialogResult==0) {
            Game.actEffId=35;
            Game.ActEffMonSource=SumMonster;
    ...
    public static void banisherHandTrapEffectExecute (int cardNumberInGY, boolean isUsedByPlayer) {
        if (Hand.hasCardWithCardIdOnHand(isUsedByPlayer, Card.BigBackBouncerBanisher.cardId)) {
            ...
            String message;
            if (isUsedByPlayer) {
                message = "You discard " + HandCard.cardName + ".";
            }
            else {
                message = "Computer discards " + HandCard.cardName + ".";
            }
            YuGiOhJi.informationDialog(message, "");
