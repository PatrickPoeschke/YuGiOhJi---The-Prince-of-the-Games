...
public class AIinterrupts {
    ...
    public static boolean cpuWouldGetAValuableMonsterTargettedByAnEffect() {
        ...
        if (Game.ActEffMonTarget.sumMonsterNumber!=0) {
            return (!Game.ActEffMonTarget.isPlayersMonster && Game.ActEffMonTarget.AIinfoThreatLv==3);
        }
        return false;
    }
    ...
    // This is not perfect yet: The player might simply attack in a different pattern and make more damage,
    // causing the CPU to discard Big Attack Stopper sometimes a bit too late and taking unnecessary damage.
    // But it is good enough for now. The way to improove this, would be to simulate all possible attack patterns of the current battle phase. (I'm too lazy to do that.)
